...
Os quatro nós à esquerda ($X_1$ a $X_4$) representam simplesmente os valores dos [dados de entrada]{.hl} da rede.

Os cinco nós azuis na coluna do meio ($A_1$ a $A_5$) formam a camada onde a maior parte da computação acontece.

O nó cor-de-rosa à direita produz o [valor de saída]{.hl} da rede.
...
A função $h_1$ começa calculando uma [combinação linear]{.hl} dos elementos do vetor
...
A função $g$ é chamada de [função de ativação]{.hl}, e costuma ser uma função [não-linear]{.hl}, como, por exemplo, a função sigmóide.
